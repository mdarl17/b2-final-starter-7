...
RSpec.describe "merchant coupon show page" do 
  before(:each) do 
    ...
    @m1_5AND5 = Coupon.create!(name: "$5 off an order of $10 or more", code: "M1_5AND5", amount: 5, discount_type: 0, status: "active", merchant_id: @m3.id)
    @m1_25OFF = Coupon.create!(name: "25% off any item we sell", code: "M1_25OFF", amount: 25, discount_type: 1, status: "inactive", merchant_id: @m3.id)
  ...
  it "has a button that activates a coupon's status" do 
    # start with deactivated coupon on coupon show page
    visit merchant_coupon_path(@m3.id, @m1_25OFF.id)

    expect(page).to have_content("Status: inactive")
    expect(page).to have_button("Activate Coupon")
    expect(page).to have_content("Code: M1_25OFF")

    click_button "Activate Coupon"

    expect(current_path).to eq(merchant_coupon_path(@m3.id, @m1_25OFF.id))

    expect(page).to have_content("Status: active")
    expect(page).to have_content("M1_25OFF has been reactivated")
  end
